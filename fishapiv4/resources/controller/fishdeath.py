...
class FishDeathApi(Resource):
    def put(self, id):
        try:
            ...
            collection = db.get_collection('fish_death')
            macthFilter = {
                "_id" : ObjectId(id)
            }
            updateFilter = {
    ...
    def delete(self, id):
        try:
            ...
            pipline = [{"$match":{"_id":id}}]
            fishdeath = db.aggregate('fish_death',pipline)
            # delete picture
            path = os.path.join(current_app.instance_path,
                                current_app.config['UPLOAD_DIR'])
    ...
    def get(self, activation_id):
        try:
            ...
            for date in list_date:
                print("dateSsSs", date)
                fishamount = FishLog.objects(pond_activation_id=activation_id,created_at__lte=date)
                print("fishamount1", fishamount)
                print("fishamount", fishamount[0].fish[0]["amount"])
                if fishamount != 0:
                    list_graph.append({'date':date.strftime("%d %b"), 'amount':fishamount[0].fish[0]["amount"]})
