...
class FishTransferApi(Resource):
    ...
    def delete(self, id):
        try:
            ...
            collectionf = db.get_collection('fish_transfer')
            matchfilterf ={
                "_id" : ObjectId(id)
            }
            # delete fish
            ...
            # ])
            fishes = db.aggregate('fish_log',[
                {'$match': {
                    '$expr': {'$eq': ['$fish_transfer_id', {'$toObjectId': id}]}}},
            ])
            for fish in fishes:
                # fishlog = FishLog.objects.get(id=fish['_id'])
                collections = db.get_collection('fish_log')
                matchfilters={
                "_id" : ObjectId(fish['_id'])
                }
                collections.delete_one(matchfilters)
                # fishlog.delete()
            # delete data
            # fishtransfer.delete()
            collectionf.delete_one(matchfilterf)
