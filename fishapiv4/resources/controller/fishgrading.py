...
from fishapiv4.database.models import FishGrading, Pond, PondActivation, FeedHistory
    ...
    def post(self):
        try:
            ...
            def _getTotalWeightFromListFishNow(listFish, avg_weight):
                totalWeight = 0
                for fish in listFish:
                    weight = float(fish['amount']) * float(avg_weight)
                    totalWeight += weight
                return totalWeight
            ...
            theDate = request.form.get('grading_at', None)
            ...
            print("diff_feed_dose", diff_feed_dose)
            print("diff_fish_weight", diff_fish_weight)
            ...
            if theDate != '':
                fishGradingNewData['created_at'] = datetime.datetime.strptime(theDate, "%Y-%m-%dT%H:%M:%S.%f %z")
                fishGradingNewData['grading_at'] = datetime.datetime.strptime(theDate, "%Y-%m-%dT%H:%M:%S.%f %z") 
            else :
                three_months_ago = datetime.datetime.now() - datetime.timedelta(days=3 * 30)
                fishGradingNewData['created_at'] = three_months_ago
                fishGradingNewData['grading_at'] = three_months_ago
            fishGradingNew = FishGrading(**fishGradingNewData).save(using=current_app.config['CONNECTION'])

            response = {"message": "Berhasil add grading", "id" : fishGradingNew.id}
            response = json.dumps(response, default=str)
            return Response(response, mimetype="application/json", status=200)
        except Exception as e:
            response = {"message": str(e)}
...
class FishGradingApi(Resource):
    def put(self, id):
        try:
            ...
            collection = db.get_collection('fish_grading')
            macthFilter = {
                "_id" : ObjectId(id)
            }
            updateFilter = {
...
class FishGradingGraphApi(Resource):
    def get(self, activationOrPondId):
        ...
        print("daybetweendate", day_beetween_date)
        ...
        for day in day_beetween_date:
            ...
            print("fish grading fish", fish_grading)
