...
# from fishapiv4.resources.controller.authentication import token_req
...
class StatisticApi(Resource):
    ...
    def get(self):
        try:
            ...
            # fish live
            total_fish_live = 0
            tempFishLive = FishGrading.objects(
                pond_activation_id__in=id_active_culture_season)
            for item in tempFishLive:
                print("event desc", item.event_desc)
                if (item.event_desc != "DEATH"):
                    print("amount", item.fish[0]['amount'])
                    total_fish_live += int(item.fish[0]['amount'])
            print("total_fish_live", total_fish_live)
            # total_fish_live = 0

            # fish death
            total_fish_death = 0
            tempFishDeath = FishDeath.objects(
                pond_activation_id__in=id_active_culture_season)
            print("tempfishdeath", tempFishDeath)
            for item in tempFishDeath:
                print("amount", item.fish[0]['amount'])
                total_fish_death += int(item.fish[0]['amount'])
            print("total_fish_death", total_fish_death)
            ...
            response = {
                ...
                "fish_live": total_fish_live - total_fish_death,
