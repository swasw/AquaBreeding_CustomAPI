...
def activationPond(args,transfer, isFishLogUpdate=True):
    ...
    pipline=[{"$match":{"_id":transfer['destination_pond_id'] }}]
    ...
    list_pond_year = db.aggregate('pond_activation',pipeline_year)
    list_pond_year = list(list_pond_year)
    id_int = len(list_pond_year) + 1
    collection = db.get_collection('pond_activation')
    body = {
        "id_int": id_int,
    ...
    pond_activation = collection.insert_one(body)
...
#     pond_activation = db.aggregate('pond_activation',pipeline)
    ...
    pond_activation = collection.update_one(matchfilter,updatefilter)
    ...
    body = {"isActive": False,
        "status": "Aktif",  "pondDoDesc": "Belum Diukur", "pondPhDesc": "Belum Diukur", "pondPh": None, "pondDo": None, "pondTemp": None}
    matchfilter={
        "_id":args['origin_pond_id']
    }
    updatefilter={
        "$set":body
    }
    collection.update_one(matchfilter,updatefilter)
